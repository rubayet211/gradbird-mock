...
export function calculateScore(sessionData, mockTestData) {
    ...
    // Calculate Listening Score using MockTest schema: listening.questions[]
    if (mockTestData.listening?.questions) {
        let correctCount = 0;
        let totalCount = 0;

        mockTestData.listening.questions.forEach(question => {
            const qId = question.id;
            const correctAnswer = question.correctAnswer;
