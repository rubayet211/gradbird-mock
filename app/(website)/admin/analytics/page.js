...
async function getAnalyticsData() {
    ...
    // ===== SPEAKING ANALYTICS =====
    const now = new Date();
    const [totalSlots, bookedSlots, upcomingSlots] = await Promise.all([
    ...
    // ===== MOCK TEST ANALYTICS =====
    const [activeMocks, inactiveMocks, mocksByType] = await Promise.all([
        MockTest.countDocuments({ isActive: true }),
        MockTest.countDocuments({ isActive: false }),
        MockTest.aggregate([{ $group: { _id: '$type', count: { $sum: 1 } } }])
    ]);
        ...
        <div className="max-w-7xl mx-auto">
            <div className="mb-8">
                ...
                <p className="text-gray-500 mt-1">
                    Detailed insights into tests, scores, payments, and more
                </p>
            </div>
